...
namespace lightstreamer::client::transport {
    ...
    public:
        ...
        std::shared_ptr<RequestHandle> sendRequest(std::shared_ptr<protocol::Protocol> protocol,
                                                   std::shared_ptr<requests::LightstreamerRequest> request,
                                                   std::shared_ptr<RequestListener> protocolListener,
                                                   std::unordered_map<std::string, std::string> extraHeaders,
                                                   Proxy proxy, long tcpConnectTimeout, long tcpReadTimeout) {
            if (!httpProvider) {
            ...
            try {
                ...
                connection = httpProvider->createConnection(protocol, request, httpListener, extraHeaders, proxy,
                                                            tcpConnectTimeout, tcpReadTimeout);
    ...
    private:
        ...
        public:
            MyHttpListener(std::shared_ptr<RequestListener> lst, std::shared_ptr<requests::LightstreamerRequest> req,
                           std::shared_ptr<session::SessionThread> thread)
