...
#include <lightstreamer/client/requests/ForceRebindRequest.hpp>
#include <lightstreamer/client/requests/DestroyRequest.hpp>
#include <lightstreamer/client/requests/ConstrainRequest.hpp>
...
namespace lightstreamer::client::protocol {
    ...
    public:
        ...
        virtual void sendForceRebind(const requests::ForceRebindRequest& request, const std::shared_ptr<requests::RequestTutor>& tutor) = 0;

        virtual void sendDestroy(const requests::DestroyRequest& request, const std::shared_ptr<requests::RequestTutor>& tutor) = 0;

        virtual void sendConstrainRequest(const requests::ConstrainRequest& request, const std::shared_ptr<session::Session::ConstrainTutor>& tutor) = 0;

        virtual void sendCreateRequest(const requests::CreateSessionRequest& request) = 0;

        virtual std::shared_ptr<util::ListenableFuture> sendBindRequest(const requests::BindSessionRequest& request) = 0;

        virtual void stop(bool waitPendingControlRequests, bool forceConnectionClose) = 0;

        virtual void sendMessageRequest(const requests::MessageRequest& request, const std::shared_ptr<requests::RequestTutor>& tutor) = 0;

        virtual void sendSubscriptionRequest(const requests::SubscribeRequest& request, const std::shared_ptr<requests::RequestTutor>& tutor) = 0;

        virtual void sendUnsubscriptionRequest(const requests::UnsubscribeRequest& request, const std::shared_ptr<requests::RequestTutor>& tutor) = 0;

        virtual void sendConfigurationRequest(const requests::ChangeSubscriptionRequest& request, const std::shared_ptr<requests::RequestTutor>& tutor) = 0;

        virtual void sendReverseHeartbeat(const requests::ReverseHeartbeatRequest& request, const std::shared_ptr<requests::RequestTutor>& tutor) = 0;
        ...
        virtual std::shared_ptr<util::ListenableFuture> openWebSocketConnection(const std::string& serverAddress) = 0;

        /// Forward the session recovery request to the transport layer.
        virtual void sendRecoveryRequest(const requests::CreateSessionRequest& request) = 0;
