...
namespace Lightstreamer::Cpp::LightstreamerClient {
    ...
    \ref `Subscription` handling and to send messages.
    ...
    \ref `.ClientListener` or \ref `.SubscriptionListener`.
    ...
    @param serverAddress: the address of the Lightstreamer Server to which this LightstreamerClient will connect to.
     It is possible to specify it later by using None here. See \ref `ConnectionDetails.setServerAddress` for details.
    @param adapterSet: the name of the Adapter Set mounted on Lightstreamer Server to be used to handle all requests
     ...
     later by using None here. See \ref `ConnectionDetails.setAdapterSet` for details.

    @throw IllegalArgumentException: if a not valid address is passed. See \ref `ConnectionDetails.setServerAddress`
     for details.
    **/
    ...
    public:
        ...
        **/
        ...
        When connect() is called, unless a single transport was forced through
                \ref `ConnectionOptions.setForcedTransport`, the so called "Stream-Sense" mechanism is started:
        ...
        **lifecycle** Note that the request to connect is accomplished by the client in a separate thread; this means
        ...
        @throw IllegalStateException: if no server address was configured.
        ...
        **/
        ...
        See \ref `addCookies` for clarifications on when cookies are directly stored by the library and when not.
        ...
        **/
        ...
        /**
		Returns a list containing the \ref `.ClientListener` instances that were added to this client.
        ...
        **/
        ...
        will be notified to the listener through \ref `.ClientMessageListener.onDiscarded`.
        ...
        **lifecycle** Since a message is handled by the Metadata Adapter associated to the current connection, a
        ...
        the message will be abandoned and the \ref `.ClientMessageListener.onAbort` event will be fired.
        ...
        **/
		...
		Static method that permits to configure the logging system used by the library. The logging system must respect
         the \ref `.LoggerProvider` interface. A custom class can be used to wrap any third-party logging system.
        ...
        * lightstreamer.actions: logs settings / API calls.
        ...
        @param provider: A \ref `.LoggerProvider` instance that will be used to generate log messages by the library
        classes.
        **/
        ...
        **lifecycle** May be called only once before creating any LightstreamerClient instance.
        ...
        @param factory: an instance of ssl.SSLContext
                ...
                @throw IllegalStateException: if a factory is already installed
        **/
        ...
        **lifecycle** Subscriptions can be given to the LightstreamerClient at any time. Once done the Subscription
        ...
        Once "active", a Subscription instance cannot be provided again to a LightstreamerClient unless it is
                first removed from the "active" state through a call to \ref `unsubscribe`.

        Also note that forwarding of the subscription to the server is made in a separate thread.

        A successful subscription to the server will be notified through a \ref `.SubscriptionListener.onSubscription`
        event.
        ...
        **lifecycle** Subscription can be unsubscribed from at any time. Once done the Subscription immediately
        ...
        The unsubscription will be notified through a \ref `.SubscriptionListener.onUnsubscription` event.
